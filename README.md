...
- **무료 TTS 목소리 26종** (`#/voice`): 한국어 기본 9종(선희·인준·현수·봉진·국민·지민·서현·순복·유진) + 스타일 변형 10종(뉴스·동화 낭독·다큐·쇼츠 빠르게·광고·속삭임·스포츠 중계·예능·강의·힐링) + 영어/일본어/중국어 7종. 목소리마다 **▶ 듣기**로 바로 들어보고, 쇼츠 "AI 후킹 보이스"와 AI 재구성 내레이션에서 고를 수 있습니다. 서버에 `edge-tts`(무료 Microsoft 신경망 음성 CLI, `pip install edge-tts`)가 있으면 실제 MP3 를 만들고, 없으면 브라우저/OS 내장 음성(Web Speech API, 무료·오프라인)으로 재생합니다.
