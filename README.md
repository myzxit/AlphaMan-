...
| 환경 | 원본 | 방법 |
|---|---|---|
| 웹사이트 (Vercel, ffmpeg 없음) | 업로드 파일 | **🎬 영상 파일 만들기** → 브라우저 렌더러(`render-browser.js`)가 원본을 재생하며 원본 박힌 자막 크롭 · 새 자막 · 후킹 배너 · 구독 카드 · TTS 내레이션 믹싱을 캔버스/WebAudio 로 그려 MediaRecorder 로 녹화(MP4 또는 WebM). 영상 길이만큼 걸리며 결과는 내려받거나 조각 업로드(`POST /api/library/:id/render`)로 보관함에 저장 → 미리보기/공유가 렌더 파일을 재생 |
| 웹사이트 | 유튜브 링크 | 브라우저는 유튜브 프레임을 읽을 수 없어 미리보기(타임라인 재생)까지만. 같은 작업을 PC 프로그램에서 열면 렌더링됨 |
| PC 프로그램 | 링크 · 파일 · USB | 첫 실행 시 **ffmpeg/ffprobe/yt-dlp 를 자동 설치**(사용자 데이터 폴더 `bin/`)해 실제 MP4 렌더링: 유튜브 원본 내려받기 → 박힌 자막 크롭 → 새 자막 굽기(libass) → TTS 내레이션 믹싱 → 구독 카드. 상단 배지에 설치 진행률 표시 |
| 자체 호스팅 (Docker) | 모두 | 이미지에 ffmpeg + yt-dlp 포함 |

- **TTS 음성 파일**: 무료 목소리 26종은 서버(웹사이트 서버리스 포함)에서 Edge 읽어주기 서비스로 MP3 를 만듭니다(외부 패키지 없이 WebSocket 직접 구현, 버전 거부 시 자동으로 다음 버전 시도). 실패하면 Google 읽어주기 → 브라우저 음성 순으로 대체합니다.
- **썸네일 이미지**: `image.svg` 는 배경 사진을 base64 로 안에 넣어 `<img>` 로 바로 보이며, 브라우저에서 캡처한 장면은 Blob 에도 보관됩니다.
