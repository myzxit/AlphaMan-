...
import { esc, html, raw, toast, modal, confirmDialog, fmtTime, fmtNum, fmtDate, fmtKRW, compactViews, readVideoMeta, qs, qsa, on, debounce } from './ui.js';
...
export const studio = auth(async ({ view, state, navigate }) => {
  const [{ templates, genres, ratios }, jobs, targets] = await Promise.all([get('/api/shorts/templates'), get('/api/shorts/jobs'), get('/api/translate/targets')]);
  const feats = state.info.content.features.items;
  view.innerHTML = html`<div class="row row-between"><h1>쇼츠 스튜디오</h1><span class="badge">보유 이용권 ${state.user.credits}분</span></div>
  ...
  qs('#s-go').onclick = async (e) => {
    const options = { template: tpl, ratio: qs('#s-ratio').value, genre: qs('#s-genre').value || undefined, clipCount: qs('#s-count').value, targetLanguages: [...langs], language: 'ko' };
