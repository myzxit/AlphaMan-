...
// 파일 관리자 → "프로젝트에 추가": ?upload=ID 로 들어오면 이미 올린 파일을 바로 선택 상태로 만든다
export async function prefillUpload(id) {
  if (!id) return null;
...
export const dashboard = auth(async ({ view, state }) => {
  const [jobs, projects, queue, notifs, lib] = await Promise.all([get('/api/shorts/jobs'), get('/api/subtitles/projects'), get('/api/publish/queue'), get('/api/notifications'), get('/api/library').catch(() => ({ items: [], stats: { total: 0 } }))]);
  const disc = await get('/api/discovery/home');
  view.innerHTML = html`<h1>${disc.greeting}</h1><p class="muted">${disc.subtitle}</p>
    <div class="grid grid-5"><div class="card"><div class="muted small">보유 이용권</div><div style="font-size:1.6rem;font-weight:900">${creditsLabel(state.user)}</div><a href="#/credits" class="small">충전 / 내역</a></div><div class="card"><div class="muted small">보관함 (내 영상)</div><div style="font-size:1.6rem;font-weight:900">${lib.stats.total}</div><a href="#/library" class="small">미리보기 · 저장</a></div><div class="card"><div class="muted small">쇼츠 작업</div><div style="font-size:1.6rem;font-weight:900">${jobs.length}</div><a href="#/studio" class="small">스튜디오</a></div><div class="card"><div class="muted small">자막 프로젝트</div><div style="font-size:1.6rem;font-weight:900">${projects.length}</div><a href="#/subtitles" class="small">편집기</a></div><div class="card"><div class="muted small">예약 업로드</div><div style="font-size:1.6rem;font-weight:900">${queue.filter((q) => q.status === 'scheduled').length}</div><a href="#/publish" class="small">SNS 업로드</a></div></div>
    <div class="section-head" style="text-align:left;margin-top:28px"><h2>바로 시작</h2></div>
    <div class="grid grid-4">${raw([['#/studio', '✂️ 쇼츠 만들기', '링크나 파일로 하이라이트 쇼츠'], ['#/remix', '🪄 AI 재구성', '자막·효과음 새로 입혀 1~28분 재구성'], ['#/library', '📁 보관함', '내가 만든 영상 미리보기 · MP4 저장'], ['#/voice', '🎤 내 목소리 TTS', '내 목소리로 후킹·내레이션'], ['#/subtitles', '💬 자막 만들기', 'Whisper STT + 파형 편집'], ['#/topic', '📈 주제 추천', '채널 분석 & 떡상 주제'], ['#/discovery', '🔥 디스커버리', 'KR 쇼츠 급상승·성장 채널']].map(([h, t, b]) => `<a class="entry-card" href="${h}"><div>${esc(t)}</div><div class="small muted" style="font-weight:500">${esc(b)}</div></a>`).join(''))}</div>
    ${lib.items.length ? raw(`<div class="section-head" style="text-align:left;margin-top:28px"><h2>최근 만든 영상</h2></div><div class="grid grid-4">${lib.items.slice(0, 4).map((it) => `<div class="card clip-card"><div class="clip-preview r-${(it.ratio || '16:9').replace(':', '-')}" style="${it.thumbnail ? `background-image:url('${esc(it.thumbnail)}')` : ''};max-height:200px;cursor:pointer;display:grid;place-items:center" data-preview="${it.id}"><div class="library-play">▶</div><div class="zoom">${fmtTime(it.durationSec)}</div></div><b>${esc(it.title)}</b><div class="tiny muted">${esc({ shorts: '쇼츠', remix: 'AI 재구성', longform: '롱폼 컷편집' }[it.kind] || it.kind)} · ${fmtDate(it.savedAt)}</div><div class="row"><button class="btn btn-sm btn-primary" data-preview="${it.id}">▶ 미리보기</button><a class="btn btn-sm" href="#/library">보관함</a></div></div>`).join('')}</div>`) : ''}
...
// ---------------- 쇼츠 스튜디오 ----------------
export const studio = auth(async ({ view, state, navigate, query }) => {
  ...
  async function pick(file, metaKnown) {
    try {
      ...
      tc.fromFile(file); // 업로드와 동시에 브라우저에서 원본 대본 추출
  ...
  if (query.upload) { const m = await prefillUpload(query.upload); if (m) { qsa('#s-tabs .tab')[1].click(); uploadId = m.id; qs('#s-file-meta').textContent = `${m.name} · ${fmtTime(m.durationSec)} · 파일 관리자에서 선택됨 (업로드 완료)`; qs('#s-upload-bar').style.width = '100%'; } }
...
export const studioJob = auth(async ({ view, params, state, navigate }) => {
  ...
  const draw = () => {
    ...
    view.innerHTML = html`<div class="row row-between"><div><a href="#/studio" class="small">← 스튜디오</a><h1>${job.source.title}</h1><div class="muted small">${job.source.type === 'youtube' ? raw(`<a href="${esc(job.source.url)}" target="_blank" rel="noopener">${esc(job.source.url)}</a>`) : '업로드 파일'} · ${fmtTime(job.source.durationSec)} · ${job.minutesCharged}분 차감 · 장르: ${job.genre || '-'} · 재생성 ${job.regenerations}회 ${raw(exactBadge(job.transcriptExact))}${job.transcriptEngine ? raw(` <span class="badge badge-soft">STT ${esc(job.transcriptEngine)}</span>`) : ''}</div></div>
      <div class="row">${job.status === 'done' ? raw('<a class="btn" href="#/library?kind=shorts">📁 보관함</a>') : ''}<button class="btn" id="j-regen" ${job.status === 'done' || job.status === 'failed' ? '' : 'disabled'}>재생성하기 (${(job.minutesCharged / 2).toFixed(1)}분)</button><button class="btn btn-danger" id="j-del">삭제</button></div></div>
      ${job.status === 'done' ? raw('<div class="card small muted" style="margin-top:12px">✅ 완성된 쇼츠는 보관함에 자동 저장되었습니다. 각 클립의 <b>▶ 미리보기</b>로 완성본(구간 · 자막 · 후킹)을 바로 확인하세요.</div>') : ''}
...
function editClipModal(clip, templates, ratios, onSaved) {
  const m = modal(html`<div class="field"><label>제목</label><input id="e-title" value="${clip.title}" /></div>
    ...
    <label class="check"><input type="checkbox" id="e-burned" ${clip.cropBottom ? 'checked' : ''}/> 원본에 박힌 자막 가리기 (하단 크롭)</label>
...
export const longform = auth(async ({ view, params, state, navigate, query }) => {
  ...
  view.innerHTML = html`<h1>롱폼 컷편집</h1><p class="muted">롱폼 영상 전체의 무음 구간을 제거하고 자동 자막·챕터를 만듭니다. (원본 길이만큼 이용권 차감)</p>
    <div class="split"><div class="card"><div class="field"><label>유튜브 링크 또는 업로드</label><div class="input-row"><input id="l-url" placeholder="https://www.youtube.com/watch?v=..." /><input type="file" id="l-file" accept="video/*" style="max-width:220px" /></div></div>
      ...
      <div class="field" style="margin-top:8px"><label>무음 기준(초)</label><input id="l-th" type="number" step="0.1" value="0.7" /></div>
      ${raw(transcriptPanel('l', { info: state.info }))}
      <button class="btn btn-primary" id="l-go" style="margin-top:10px">컷편집 시작</button>
  ...
  qs('#l-go').onclick = async (e) => {
    ...
    try {
      let body = { url: qs('#l-url').value, options, ...tc.payload() };
...
// ---------------- 자막 편집기 (픽셀링) ----------------
export const subtitles = auth(async ({ view, state, navigate, query }) => {
  ...
  qs('#p-file-input').onchange = async (e) => { const f = e.target.files[0]; if (!f) return; try { const meta = f.type.startsWith('audio') ? null : await readVideoMeta(f); tc.fromFile(f); qs('#p-file-meta').textContent = `${f.name} 업로드 중...`; const up = await uploadFile(f); uploadId = up.id; qs('#p-file-meta').textContent = `${f.name} 업로드 완료${meta ? ` · ${fmtTime(meta.durationSec)}` : ''}`; } catch (err) { toast(err.message, 'error', 6000); } };
  if (window.alphaman?.pickVideo) qs('#p-pick-local').onclick = async () => { const p = await window.alphaman.pickVideo(); if (p) { localPath = p; qs('#p-local-path').textContent = p; tc.fromLocalPath(p); } };
  if (query.upload) { const m = await prefillUpload(query.upload); if (m) { qsa('#p-tabs .tab')[1].click(); uploadId = m.id; qs('#p-file-meta').textContent = `${m.name} · 파일 관리자에서 선택됨 (업로드 완료)${m.durationSec ? ` · ${fmtTime(m.durationSec)}` : ''}`; } }
  qs('#p-go').onclick = async (e) => {
    if (tc.state.busy) return toast('원본 대본 추출이 끝날 때까지 잠시 기다려주세요.', 'info');
    const body = { language: qs('#p-lang').value, premium: Boolean(qs('#p-premium').value), ...tc.payload() };
...
export const subtitleEditor = auth(async ({ view, params, state, navigate, query }) => {
  ...
  const syncHist = () => { hist = { ...hist, undo: (p.history || []).length, redo: (p.future || []).length }; const u = qs('#ed-undo'); const r = qs('#ed-redo'); if (u) { u.textContent = `↶ 되돌리기 (${hist.undo})`; u.disabled = !hist.undo; } if (r) { r.textContent = `↷ 다시 실행 (${hist.redo})`; r.disabled = !hist.redo; } };
  // 자동 저장 (설정에서 끌 수 있음) + 새로고침/네트워크 끊김 복구용 로컬 초안
  const autosave = createAutosave({ key: `subtitle:${p.id}`, enabled: prefs.autosave !== false, statusEl: () => qs('#ed-autosave'), save: async (segments) => { p = await put(`/api/subtitles/projects/${p.id}/segments`, { segments }); syncHist(); } });
  // 마지막 작업 위치 (프로젝트 페이지의 "이어서 열기")
  const savePos = debounce(() => put(`/api/projects/subtitle/${p.id}/position`, { position: { playhead, segIndex: focusIdx, lang, at: Date.now() } }).catch(() => {}), 1500);
  const draw = () => {
    const font = fonts.find((f) => f.id === p.style.font) || fonts[0];
    const s = p.style; const list = segs(); const f = filter.toLowerCase();
    view.innerHTML = html`<div class="row row-between"><div><a href="#/subtitles" class="small">← 프로젝트</a><h1>${p.projectTitle || p.source.title}</h1><div class="tiny muted">${fmtTime(p.source.durationSec)} · STT: ${p.engine} ${raw(exactBadge(p.transcriptExact))} · 파형: ${p.waveform?.engine} · ${p.premium ? '프리미엄' : '기본'} · <span id="ed-autosave" class="autosave-status"></span></div></div>
      <div class="row"><select id="ed-lang" style="width:auto" aria-label="언어"><option value="">원문 (${p.language})</option>${raw(Object.keys(p.translations).map((l) => `<option value="${l}" ${l === lang ? 'selected' : ''}>${esc(l)} 번역</option>`).join(''))}</select>
        <select id="ed-export" style="width:auto" aria-label="내보내기 형식"><option value="srt">SRT</option><option value="vtt">VTT</option><option value="ass">ASS</option><option value="txt">TXT</option><option value="json">JSON</option></select><button class="btn btn-primary" id="ed-download">내보내기</button><button class="btn" id="ed-versions" title="버전 기록">🕘 버전</button></div></div>
      ${p.notice ? raw(`<div class="notice-bar" style="border-radius:10px;margin-bottom:12px">${esc(p.notice)}</div>`) : ''}
      <div class="split editor-split"><div class="col">
        <div class="card"><div class="wave-wrap"><canvas id="wave" class="waveform" width="1200" height="120" style="width:${zoom * 100}%" aria-label="파형 (클릭·터치로 이동, 핀치/슬라이더로 확대)"></canvas></div><div class="row row-between tiny muted"><span>00:00</span><span id="ed-playhead">재생 위치 ${fmtTime(playhead)} · 파형 클릭으로 이동, 선택 자막에서 분할</span><span class="row" style="gap:4px">확대 <input id="ed-zoom" type="range" min="1" max="8" step="0.5" value="${zoom}" style="width:80px" aria-label="타임라인 확대" /> ${fmtTime(p.source.durationSec)}</span></div>
          <div class="row editor-tools" style="margin-top:8px"><button class="btn btn-sm" id="ed-split">선택 자막 분할 @재생위치</button><button class="btn btn-sm" id="ed-merge">선택 병합</button><button class="btn btn-sm" id="ed-resplit">의미 기반 재분할</button><button class="btn btn-sm" id="ed-undo" title="Ctrl+Z">↶ 되돌리기 (${hist.undo})</button><button class="btn btn-sm" id="ed-redo" title="Ctrl+Shift+Z">↷ 다시 실행 (${hist.redo})</button><button class="btn btn-sm" id="ed-history" title="변경 기록 보기">기록</button><button class="btn btn-sm" id="ed-add">자막 추가</button><button class="btn btn-sm" id="ed-import">SRT/VTT 가져오기</button><button class="btn btn-sm" id="ed-replace">찾기/바꾸기</button><button class="btn btn-sm" id="ed-shift">시간 이동</button><button class="btn btn-sm" id="ed-batch" ${selected.size ? '' : 'disabled'}>선택 ${selected.size}개 일괄 편집</button></div></div>
        <div class="card"><div class="row row-between" style="margin-bottom:8px"><input id="ed-search" value="${filter}" placeholder="자막 문장 검색 (Enter 로 첫 결과로 이동)" aria-label="자막 검색" style="max-width:320px" /><span class="tiny muted">${list.length}줄${f ? ` · 검색 ${list.filter((x) => x.text.toLowerCase().includes(f)).length}건` : ''}${selected.size ? ` · 선택 ${selected.size}` : ''}</span></div>
          <div id="seg-list">${raw(list.map((sg, i) => (f && !sg.text.toLowerCase().includes(f) ? '' : `<div class="seg-row ${selected.has(sg.id) ? 'active' : ''}" data-id="${sg.id}" id="segrow-${esc(sg.id)}"><input data-f="start" data-i="${i}" value="${sg.start}" type="number" step="0.01" aria-label="시작" /><input data-f="end" data-i="${i}" value="${sg.end}" type="number" step="0.01" aria-label="끝" /><input data-f="text" data-i="${i}" value="${esc(sg.text)}" aria-label="자막 ${i + 1}" /><div class="row" style="gap:4px"><button class="btn btn-sm" data-sel="${sg.id}" aria-pressed="${selected.has(sg.id)}">${selected.has(sg.id) ? '✓' : '선택'}</button><button class="btn btn-sm btn-danger" data-rm="${i}" aria-label="삭제">✕</button></div></div>`)).join(''))}</div><div class="row" style="margin-top:10px"><button class="btn btn-primary" id="ed-save" title="Ctrl+S">자막 저장</button><span class="tiny muted" id="ed-dirty"></span></div></div></div>
      <div class="col sticky"><div class="card"><h3>미리보기</h3><div class="subtitle-preview"><div class="line" id="pv-line" style="font-family:'${font.family}';font-size:${Math.round(s.size / 3)}px;color:${s.color};background:${s.bg || 'transparent'};-webkit-text-stroke:${s.outline ? `${Math.max(0.5, s.outlineWidth / 4)}px ${s.outline}` : '0'};font-weight:900">${list[0]?.text || '자막 미리보기'}</div></div></div>
        <div class="card"><div class="row row-between"><h3>스타일</h3><div class="row" style="gap:4px"><button class="btn btn-sm" id="st-tpl-apply" title="저장된 템플릿 적용">템플릿 적용</button><button class="btn btn-sm" id="st-tpl-save" title="현재 스타일을 템플릿으로 저장">템플릿으로 저장</button></div></div><div class="field"><label>프리셋</label><div class="chips">${raw(presets.map((pr) => `<span class="chip ${pr.id === s.presetId ? 'active' : ''}" data-preset="${pr.id}" role="button" tabindex="0">${esc(pr.name)}</span>`).join(''))}</div></div>
    ...
    drawWave(); autosave.refresh(); syncHist();
    qs('#ed-lang').onchange = (e) => { lang = e.target.value || null; draw(); };
    qs('#ed-zoom').oninput = (e) => { zoom = Number(e.target.value); qs('#wave').style.width = `${zoom * 100}%`; };
    qs('#ed-search').onkeydown = (e) => { if (e.key === 'Enter') { e.preventDefault(); filter = e.target.value.trim(); draw(); const first = qs('#seg-list .seg-row'); if (first) first.querySelector('[data-f="text"]').focus(); } };
    qs('#ed-search').oninput = debounce((e) => { filter = e.target.value.trim(); draw(); qs('#ed-search').focus(); const el = qs('#ed-search'); el.setSelectionRange(el.value.length, el.value.length); }, 350);
    ...
    qs('#ed-versions').onclick = async () => { const { openVersions } = await window.AlphaManApp.loadWorkspace(); openVersions('subtitle', p.id); };
    ...
    on(view, 'focusin', '[data-f]', (e, t) => { const sg = p.segments[Number(t.dataset.i)]; if (sg) { focusIdx = Number(t.dataset.i); qs('#pv-line').textContent = sg.text; playhead = sg.start; qs('#ed-playhead').textContent = `재생 위치 ${fmtTime(playhead)}`; drawWave(); savePos(); } });
    ...
    qs('#ed-import').onclick = () => { const m = modal('<div class="tabs"><button class="tab active" data-fmt="srt">SRT</button><button class="tab" data-fmt="vtt">VTT</button></div><input type="file" id="imp-file" accept=".srt,.vtt,text/plain" style="margin-bottom:8px" /><textarea id="imp" rows="8" placeholder="SRT / VTT 내용 붙여넣기 또는 위에서 파일 선택"></textarea><button class="btn btn-primary btn-block" id="imp-go" style="margin-top:8px">가져오기 (현재 자막을 대체)</button>', { title: '자막 파일 가져오기' }); let fmt = 'srt'; m.el.querySelectorAll('[data-fmt]').forEach((b) => { b.onclick = () => { fmt = b.dataset.fmt; m.el.querySelectorAll('[data-fmt]').forEach((x) => x.classList.toggle('active', x === b)); }; }); m.el.querySelector('#imp-file').onchange = async (e) => { const f = e.target.files[0]; if (!f) return; m.el.querySelector('#imp').value = await f.text(); if (/\.vtt$/i.test(f.name)) m.el.querySelector('[data-fmt="vtt"]').click(); }; m.el.querySelector('#imp-go').onclick = async () => { try { const text = m.el.querySelector('#imp').value; if (/^WEBVTT/m.test(text)) fmt = 'vtt'; await autosave.flush(); p = fmt === 'vtt' ? await post(`/api/subtitles/projects/${p.id}/import-vtt`, { vtt: text }) : await post(`/api/subtitles/projects/${p.id}/import`, { srt: text }); m.close(); toast(`${fmt.toUpperCase()} 자막 ${p.segments.length}줄을 가져왔습니다.`); draw(); } catch (err) { toast(err.message, 'error'); } }; };
    qs('#ed-replace').onclick = () => { const m = modal(`<div class="grid grid-2"><div class="field"><label>찾을 문자열</label><input id="fr-find" value="${esc(filter)}" /></div><div class="field"><label>바꿀 문자열</label><input id="fr-rep" /></div></div><label class="check"><input type="checkbox" id="fr-regex"/> 정규식</label><label class="check"><input type="checkbox" id="fr-case"/> 대소문자 구분</label><label class="check"><input type="checkbox" id="fr-sel" ${selected.size ? '' : 'disabled'}/> 선택한 ${selected.size}개 자막에서만</label><div class="row" style="margin-top:8px"><button class="btn" id="fr-preview">미리보기</button><button class="btn btn-primary" id="fr-go">모두 바꾸기</button></div><div id="fr-out" class="small" style="margin-top:8px"></div>`, { title: '찾기 / 바꾸기' }); const body = () => ({ find: m.el.querySelector('#fr-find').value, replace: m.el.querySelector('#fr-rep').value, regex: m.el.querySelector('#fr-regex').checked, caseSensitive: m.el.querySelector('#fr-case').checked, segmentIds: m.el.querySelector('#fr-sel').checked ? [...selected] : null }); m.el.querySelector('#fr-preview').onclick = () => { const b = body(); if (!b.find) return; let re; try { re = new RegExp(b.regex ? b.find : b.find.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), b.caseSensitive ? 'g' : 'gi'); } catch (err) { return toast(`정규식 오류: ${err.message}`, 'error'); } const hits = p.segments.filter((s) => (!b.segmentIds || b.segmentIds.includes(s.id)) && re.test(s.text)); m.el.querySelector('#fr-out').innerHTML = hits.length ? `${hits.length}건: ${hits.slice(0, 8).map((s) => `<div>${fmtTime(s.start)} ${esc(s.text)} → <b>${esc(s.text.replace(re, b.replace))}</b></div>`).join('')}` : '일치하는 자막이 없습니다.'; }; m.el.querySelector('#fr-go').onclick = async () => { try { await autosave.flush(); const r = await post(`/api/subtitles/projects/${p.id}/replace`, body()); p = r.project; m.close(); toast(`${r.changed}개 자막을 바꿨습니다.`); draw(); } catch (err) { toast(err.message, 'error'); } }; };
    qs('#ed-shift').onclick = () => { const m = modal(`<p class="small muted">자막 전체(또는 선택한 자막)의 시간을 한꺼번에 이동하거나 배율로 늘리고 줄입니다.</p><div class="grid grid-2"><div class="field"><label>이동(초, 음수면 앞으로)</label><input id="sh-off" type="number" step="0.1" value="0" /></div><div class="field"><label>배율 (1 = 그대로)</label><input id="sh-scale" type="number" step="0.01" value="1" /></div></div><div class="field"><label>이 시각(초) 이후만 (비우면 전체)</label><input id="sh-from" type="number" step="0.1" placeholder="예: 60" /></div><label class="check"><input type="checkbox" id="sh-sel" ${selected.size ? '' : 'disabled'}/> 선택한 ${selected.size}개 자막만</label><button class="btn btn-primary btn-block" id="sh-go" style="margin-top:8px">적용</button>`, { title: '자막 시간 이동' }); m.el.querySelector('#sh-go').onclick = async () => { try { await autosave.flush(); p = await post(`/api/subtitles/projects/${p.id}/shift`, { offsetSec: Number(m.el.querySelector('#sh-off').value), scale: Number(m.el.querySelector('#sh-scale').value), fromSec: m.el.querySelector('#sh-from').value === '' ? null : Number(m.el.querySelector('#sh-from').value), segmentIds: m.el.querySelector('#sh-sel').checked ? [...selected] : null }); m.close(); toast('시간을 이동했습니다.'); draw(); } catch (err) { toast(err.message, 'error'); } }; };
    qs('#ed-batch').onclick = () => { if (!selected.size) return; const m = modal(`<p class="small muted">선택한 ${selected.size}개 자막에 한꺼번에 적용합니다.</p><div class="field"><label>앞에 붙일 텍스트</label><input id="bt-pre" /></div><div class="field"><label>뒤에 붙일 텍스트</label><input id="bt-post" /></div><div class="row"><button class="btn" id="bt-upper">대문자로</button><button class="btn" id="bt-trim">공백 정리</button><button class="btn btn-danger" id="bt-del">선택 삭제</button></div><button class="btn btn-primary btn-block" id="bt-go" style="margin-top:8px">적용</button>`, { title: '일괄 편집' }); const apply = (fn) => { p.segments.forEach((s) => { if (selected.has(s.id)) s.text = fn(s.text); }); }; m.el.querySelector('#bt-upper').onclick = () => { apply((t) => t.toUpperCase()); m.close(); draw(); markDirty(); }; m.el.querySelector('#bt-trim').onclick = () => { apply((t) => t.replace(/\s+/g, ' ').trim()); m.close(); draw(); markDirty(); }; m.el.querySelector('#bt-del').onclick = () => { p.segments = p.segments.filter((s) => !selected.has(s.id)); selected.clear(); m.close(); draw(); markDirty(); }; m.el.querySelector('#bt-go').onclick = () => { const pre = m.el.querySelector('#bt-pre').value; const post_ = m.el.querySelector('#bt-post').value; apply((t) => `${pre}${t}${post_}`); m.close(); draw(); markDirty(); }; };
    qsa('[data-preset]').forEach((c) => { const go = async () => { p = await put(`/api/subtitles/projects/${p.id}/style`, { presetId: c.dataset.preset }); draw(); }; c.onclick = go; c.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); go(); } }; });
    const styleBody = () => ({ presetId: p.style.presetId, font: qs('#st-font').value, size: Number(qs('#st-size').value), position: qs('#st-pos').value, color: qs('#st-color').value, outline: qs('#st-outline').value, animation: qs('#st-anim').value });
    qs('#st-save').onclick = async () => { p = await put(`/api/subtitles/projects/${p.id}/style`, styleBody()); toast('스타일 저장'); draw(); };
    qs('#st-font').onchange = (e) => { const f = fonts.find((x) => x.id === e.target.value); qs('#pv-line').style.fontFamily = `'${f.family}'`; };
    qs('#st-tpl-save').onclick = async () => { const { templateDialog } = await window.AlphaManApp.loadWorkspace(); const sb = styleBody(); templateDialog({ name: `${p.projectTitle || p.source.title} 스타일`, kind: 'subtitle', settings: { subtitleStyle: { color: sb.color, outline: sb.outline, size: sb.size, position: sb.position, animation: sb.animation, font: sb.font }, templateId: p.style.presetId } }, () => toast('설정 → 템플릿에서 확인할 수 있습니다.'), state); };
    qs('#st-tpl-apply').onclick = async () => { const list = await get('/api/templates'); const usable = list.filter((t) => t.settings?.subtitleStyle || t.settings?.templateId); if (!usable.length) return toast('적용할 템플릿이 없습니다. "템플릿으로 저장"으로 먼저 만들어주세요.', 'info', 5000); const m = modal(`<div class="field"><label>템플릿</label><select id="ta-tpl">${usable.map((t) => `<option value="${t.id}">${esc(t.name)}</option>`).join('')}</select></div><button class="btn btn-primary btn-block" id="ta-go">적용</button>`, { title: '템플릿 적용' }); m.el.querySelector('#ta-go').onclick = async () => { try { const r = await post(`/api/projects/subtitle/${p.id}/apply`, usable.find((t) => t.id === m.el.querySelector('#ta-tpl').value).settings); await post(`/api/templates/${m.el.querySelector('#ta-tpl').value}/use`).catch(() => {}); p = await get(`/api/subtitles/projects/${p.id}`); m.close(); toast(`적용됨: ${(r.applied || []).join(', ') || '스타일'}`); draw(); } catch (err) { toast(err.message, 'error'); } }; };
    qs('#tr-go').onclick = async (e) => { e.target.disabled = true; e.target.textContent = '번역 중...'; try { await autosave.flush(); p = await post(`/api/subtitles/projects/${p.id}/translate`, { target: qs('#tr-target').value }); lang = qs('#tr-target').value; toast('번역 완료'); draw(); } catch (err) { toast(err.message, 'error'); e.target.disabled = false; e.target.textContent = '번역하기'; } };
    if (query.seg) { const row = document.getElementById(`segrow-${query.seg}`); if (row) { row.scrollIntoView({ block: 'center' }); row.querySelector('[data-f="text"]').focus(); } }
  };
  async function save() { if (lang) return toast('원문에서만 저장할 수 있습니다.'); try { autosave.change(p.segments); const ok = await autosave.flush(); if (ok) { toast('자막이 저장되었습니다.'); draw(); } } catch (err) { toast(err.message, 'error'); } }
  async function undo() { try { await autosave.flush(); p = await post(`/api/subtitles/projects/${p.id}/undo`); draw(); } catch (err) { toast(err.message, 'error'); } }
  async function redo() { try { await autosave.flush(); p = await post(`/api/subtitles/projects/${p.id}/redo`); draw(); } catch (err) { toast(err.message, 'error'); } }
  const markDirty = () => { const d = qs('#ed-dirty'); if (d) d.textContent = prefs.autosave === false ? '저장되지 않은 변경 사항 (Ctrl+S)' : ''; autosave.change(p.segments.map((s) => ({ ...s }))); };
  // 단축키: 저장 · 실행 취소/다시 실행 · 선택 삭제 · 복사/붙여넣기
  const unregister = registerShortcuts({
    save, undo, redo,
    delete: () => { if (!selected.size || lang) return; p.segments = p.segments.filter((s) => !selected.has(s.id)); selected.clear(); draw(); markDirty(); toast('선택한 자막을 삭제했습니다.'); },
    copy: () => { if (!selected.size) return; clipboard = segs().filter((s) => selected.has(s.id)).map((s) => ({ ...s })); navigator.clipboard?.writeText(clipboard.map((s) => s.text).join('\n')).catch(() => {}); toast(`${clipboard.length}개 자막 복사`); },
    paste: () => { if (!clipboard.length || lang) return; const at = focusIdx >= 0 ? focusIdx + 1 : p.segments.length; const base = p.segments[at - 1]?.end ?? 0; const first = clipboard[0].start; const copies = clipboard.map((s, i) => ({ ...s, id: `seg-${Date.now()}-${i}`, start: Math.round((base + 0.2 + (s.start - first)) * 100) / 100, end: Math.round((base + 0.2 + (s.end - first)) * 100) / 100 })); p.segments.splice(at, 0, ...copies); draw(); markDirty(); toast(`${copies.length}개 자막 붙여넣기`); },
  });
  const cleanup = async () => { unregister(); window.removeEventListener('hashchange', cleanup); await autosave.flush().catch(() => {}); autosave.destroy(); };
  ...
  function drawWave() {
    ...
    const seekAt = (clientX) => { const r = cv.getBoundingClientRect(); playhead = Math.round(((clientX - r.left) / r.width) * D * 100) / 100; qs('#ed-playhead').textContent = `재생 위치 ${fmtTime(playhead)}`; const hit = segs().find((s) => playhead >= s.start && playhead <= s.end); if (hit) qs('#pv-line').textContent = hit.text; drawWave(); savePos(); };
    cv.onclick = (e) => seekAt(e.clientX);
    // 터치: 한 손가락 드래그로 이동, 두 손가락 핀치로 타임라인 확대/축소
    let pinch = null;
    cv.ontouchstart = (e) => { if (e.touches.length === 2) pinch = { d: Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY), z: zoom }; };
    cv.ontouchmove = (e) => { if (e.touches.length === 2 && pinch) { e.preventDefault(); const d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY); zoom = Math.max(1, Math.min(8, pinch.z * (d / pinch.d))); cv.style.width = `${zoom * 100}%`; const z = qs('#ed-zoom'); if (z) z.value = String(zoom); } else if (e.touches.length === 1 && !pinch) { seekAt(e.touches[0].clientX); } };
    cv.ontouchend = () => { pinch = null; };
  }
  draw();
  // 새로고침/종료로 잃어버린 초안 복구 제안
  const draft = autosave.draft();
  if (draft && draft.data && draft.at > new Date(p.updatedAt).getTime() + 500 && JSON.stringify(draft.data) !== JSON.stringify(p.segments)) {
    if (await confirmDialog(`저장되지 않은 편집 내용(${new Date(draft.at).toLocaleTimeString('ko-KR')})이 이 기기에 남아 있습니다. 복구할까요?`)) { p.segments = draft.data; draw(); markDirty(); toast('초안을 복구했습니다. 자동 저장됩니다.'); } else autosave.clearDraft();
  } else if (draft) autosave.clearDraft();
  // "이어서 열기": 마지막 작업 위치로 이동
  if (query.resume) { try { const { position } = await get(`/api/projects/subtitle/${p.id}/position`); if (position) { if (position.lang && p.translations[position.lang]) { lang = position.lang; draw(); } playhead = position.playhead || 0; const rows = qsa('#seg-list .seg-row'); const row = rows[position.segIndex]; if (row) { row.scrollIntoView({ block: 'center' }); row.querySelector('[data-f="text"]').focus(); } else drawWave(); toast(`마지막 작업 위치 ${fmtTime(playhead)} 부터 이어서 편집합니다.`); } } catch { /* ignore */ } }
});
